 ...
 *
 ****************************************************************************/
static double psnr( double sqe, double size )
{
...
static double ssim_internal( double ssim )
{
                              ...
                              x264_picture_t *pic_out )
{
    ...
    *pp_nal = h->out.nal;
    ...
    if( h->param.analyse.b_psnr )
    {
        ...
        pic_out->prop.f_psnr[0] = psnr( ssd[0], luma_size );
        pic_out->prop.f_psnr[1] = psnr( ssd[1], chroma_size );
        pic_out->prop.f_psnr[2] = psnr( ssd[2], chroma_size );
        pic_out->prop.f_psnr_avg = psnr( ssd[0] + ssd[1] + ssd[2], luma_size + chroma_size*2 );
        ...
        if( h->stat.i_frame_count[i_slice] > 0 )
        {
            ...
            if( h->param.analyse.b_psnr )
            {
                x264_log( h, X264_LOG_INFO,
                          ...
                          psnr( h->stat.f_ssd_global[i_slice], dur * i_yuv_size ) );
            }
            else
            {
                x264_log( h, X264_LOG_INFO,
...
#define SUM3b(p,o) (p[SLICE_TYPE_I][o] + p[SLICE_TYPE_P][o] + p[SLICE_TYPE_B][o])
        ...
        if( h->param.analyse.b_psnr )
        {
            x264_log( h, X264_LOG_INFO,
                      ...
                      psnr( SUM3( h->stat.f_ssd_global ), duration * i_yuv_size ),
                      f_bitrate );
        }
        else
